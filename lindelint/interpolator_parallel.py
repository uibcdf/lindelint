    ...
    for jj in prange(n_points):
        ...
        if not done[jj]:
            for i in range(len(triangle)):
                ...
                if np.dot(p_to_p0, triangle_normal[i]) >= 0:
                    ...
                    p_f = point - p0
                    pts_in_b = np.array(
                        [np.dot(p_f, b01_tri[i]), np.dot(p_f, b02_tri[i])]
                    )
        ...
        if not done[jj]:
            for i in range(len(edge)):
                ...
                if (
                    np.dot(p_to_p0, op0_edge[i]) >= 0
                    and np.dot(p_to_p0, op1_edge[i]) >= 0
                ):
                    f = np.dot(p_to_p0, u_edge[i]) / d_edge[i]
                    if f >= 0 and f <= 1:
                        out_properties[jj] = (1.0 - f) * properties[
                            edge[i, 0]
                        ] + f * properties[edge[i, 1]]
                        done[jj] = True
                        break
    ...
    def _do_your_thing_3d(self, query_points):
        ...
        if np.any(inside):
            ...
            properties[inside] = np.einsum(
                "ij,ijk->ik",
                bcoords,
                self.properties[self._v_engine.delaunay.simplices[s_idx]],
            )
            done[inside] = True
