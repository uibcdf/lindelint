...
class InterpolatorVectorized:
    ...
    def _prepare_3d_structures(self):
        ...
        for ii, neighbors in enumerate(self.delaunay.neighbors):
            if -1 in neighbors:
                tetra_boundary.append(self.delaunay.simplices[ii])
        ...
        for ii, tri in enumerate(self.triangle):
            ...
            for tetra in tetra_boundary:
                if np.all(np.isin(tri, tetra)):
                    ...
                    if np.dot(self.points[interior_vertex] - p0, normal) > 0:
                        normal = -normal
            ...
            X = np.column_stack(
                [np.array(v01_in_b) - v02_in_b, np.array(v02_in_b) - v02_in_b]
            )  # Logic fix: based on original pedal
        ...
        for i, e in enumerate(self.edge):
            ...
            if len(tris) >= 2:
                ...
                if np.dot(self.points[opp_v0] - p0, op0) > 0:
                    op0 = -op0
                ...
                if np.dot(self.points[opp_v1] - p0, op1) > 0:
                    op1 = -op1
        ...
        for i, e in enumerate(self.edge):
            ...
            for tri in self.triangle:
                if np.all(np.isin(e, tri)):
                    ...
                    if np.dot(self.points[opp_v] - p0, normal) > 0:
                        normal = -normal
    ...
    def do_your_thing(self, points):
        if self.dim == 2:
            return self._do_your_thing_2d_vectorized(points)
        if self.dim == 3:
            return self._do_your_thing_3d_vectorized(points)
        return None
    ...
    def _do_your_thing_3d_vectorized(self, points):
        ...
        if np.any(inside):
            ...
            properties[inside] = np.einsum(
                "ij,ijk->ik", bcoords, self.properties[self.delaunay.simplices[s_idx]]
            )
            done[inside] = True
        ...
        for i in range(self.n_triangles):
            ...
            pts_in_b = np.stack(
                [
                    np.dot(pts_f - p0, self.b01_tri[i]),
                    np.dot(pts_f - p0, self.b02_tri[i]),
                ],
                axis=1,
            )
            b = np.einsum(
                "jk,ik->ij", self.inv_X_tri[i], pts_in_b - self.aux_vertices_last[i]
            )
            bcoords = np.column_stack([b, 1.0 - b.sum(axis=1)])
            valid = np.all((bcoords >= 0) & (bcoords <= 1), axis=1)
            if np.any(valid):
                idx = np.where(not_done)[0][in_front][valid]
                properties[idx] = np.einsum(
                    "ij,jk->ik", bcoords[valid], self.properties[self.triangle[i]]
                )
                done[idx] = True
        ...
        for i in range(self.n_edges):
            ...
            if not np.any(not_done):
                break
            if la.norm(self.outer_parallel_0[i]) == 0:
                continue
            pts = points[not_done]
            p0 = self.points[self.edge[i, 0]]
            mask = (np.dot(pts - p0, self.outer_parallel_0[i]) >= 0) & (
                np.dot(pts - p0, self.outer_parallel_1[i]) >= 0
            )
            if not np.any(mask):
                continue
            ...
            if np.any(valid):
                ...
                properties[idx] = (1.0 - f_v) * self.properties[
                    self.edge[i, 0]
                ] + f_v * self.properties[self.edge[i, 1]]
                done[idx] = True
